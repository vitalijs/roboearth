...
:- module(comp_roboearth,
    [
      ...
      re_download_action_recipe_class/2,
      re_download_model_for/2,
...
re_download_action_recipe_class(ActionClass, Robot) :-
  re_request_recipe_class(ActionClass),!,
  re_download_components_for_action(ActionClass, Robot).
...
re_download_components_for_action(Action, Robot) :-
    ...
    time(findall(MissingComp, missing_comp_for_action(Action, Robot, MissingComp), MissingComponents)),
    ...
    time(findall(ObjT, (plan_subevents_recursive(Action,Sub),
    ...
    time(findall(OwlFile, (member(ObjT, ObjTsUnique),
...
re_request_recipe_for(Command, RecipeClass) :-
  ...
  time(jpl_call('roboearth.wp5.REClients', 'requestActionRecipeFor', [Command], Array)),
  jpl_array_to_list(Array, List),
  member(RecipeURL, List),

  time(jpl_call('roboearth.wp5.REClients', 'downloadRecipeFrom', [RecipeURL], OwlFile)),
  owl_parse(OwlFile, false, false, true), print('\n'),
...
re_request_recipe_class(RecipeClass) :-

  print('\nDownloading recipe of class:'),print(RecipeClass), print('\n'),

  time(jpl_call('roboearth.wp5.REClients', 'requestActionRecipeClass', [RecipeClass], Array)),
  jpl_array_to_list(Array, List),
  member(RecipeURL, List),
