...
//        sensor_msgs::CvBridge bridge;
        ...
        capture_ = cv_bridge::toCvCopy(image_,std::string("bgr8"));
...
//        cvShowImage("win",capture_);
        ...
        dataPtr = (ARUint8 *) capture_->image.data;
...
//        int downsize = capture_->width/cloud_width_;
        int downsize = capture_->image.cols/cloud_width_;
        ...
        for (i = 0; i < objectnum; i++) {
            ...
            if(gotcloud_){
                ...
                if(!(std::isnan(point.x) || std::isnan(point.y) || std::isnan(point.z))) {
                    ...
                    tf::Quaternion q(e6p.getRot()->z,e6p.getRot()->y,e6p.getRot()->x);
        ...
        if (arPoseMarkers_.markers.size() >= 3) {
            ...
            if (transformFromModelPointsToObjectBase != NULL) {
                ...
                tf::Quaternion rotation(e6p->getRot()->z,e6p->getRot()->y,e6p->getRot()->x);
                tf::Vector3 origin (e6p->getPos()->x, e6p->getPos()->y, e6p->getPos()->z);
                tf::Transform t (rotation, origin);
