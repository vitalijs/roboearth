...
/******************************************************************************/
  ...
  {
    if (_userLambdaInit>0)   return _userLambdaInit;
...
/******************************************************************************/
  ...
  double SparseOptimizer::computeScale(double currentLMLambda, Solver* solver)
  {
    ...
    for (size_t j=0; j < solver->vectorSize(); j++)
       scale += solver->x()[j]
              * (currentLMLambda * solver->x()[j] + solver->b()[j]);
...
/******************************************************************************/
  ...
  {
    // Si existe algun vertice marginalizable => se puede usar Schur
    bool useSchur=false;
    for (VertexContainer::iterator
         it  = _activeVertices.begin();
         it != _activeVertices.end();
         it++)
    {
      OptimizableGraph::Vertex* v= *it;
      if (v->marginalized())
    ...
    // Si el resolvedor soporta Schur y se puede usar => se activa Schur
    double t;
    ...
    {
      // Calcula el error de cada edge seleccionado en initializaOptimization
      computeActiveErrors();
      // Calcula la chi2 con la chi2 cada edge seleccionado
    ...
    for (int i=0; i<iterations && ! terminate() && ok; i++)
    {
      ...
      // Construccion del sistema: Matrices Hessianas, de Schur, ...
      // Solo se hace para la primera iteracion
      double ts = get_time();
      if (i == 0 && !online)
      { // built up the CCS structure, here due to easy time measure
        ...
        if (! ok)
        {
          cerr << __PRETTY_FUNCTION__
               << ": Failure while building CCS structure\n";
          return 0;
        }
      }


      // Calculo de los errores de los edges seleccionados
      t = get_time();
      computeActiveErrors();
      if (cstat)   cstat->timeResiduals = get_time() - t;

      // Linearizacion del sistema: Calculo de los jacobianos para cada edge
      // e->linealizeOPlus()
      t = get_time();
      linearizeSystem();
      if (cstat)   cstat->timeLinearize = get_time()-t;



      // Construccion del sistema: Calculo de la Hessiana y de -b
      t = get_time();
      solver->buildSystem();
      if (cstat)   cstat->timeQuadraticForm = get_time()-t;


      // Resolucion del sistema:   H * x = -b
      if (_method == GaussNewton)
      {
        t = get_time();
        ok = solver->solve();
        if (cstat)   cstat->timeLinearSolution = get_time()-t;

        t = get_time();
        update(solver->x());
        if (cstat)   cstat->timeUpdate = get_time()-t;

      } else if (_method == LevenbergMarquardt)
      {
        if (i==0)   _currentLambda = computeLambdaInit();
        ...
        { // Metodo de LM
          ...
          if (cstat)   cstat->levenbergIterations++;
          ...
          // Actualizacion del estado de los vertices no fijos
          // v->oplus(update);
          // vertice = vertice + x, x = incremento
          t = get_time();
          update(solver->x());
          if (cstat)   cstat->timeUpdate = get_time()-t;
          ...
          // Calculo de los nuevo errores y obtencion de la nueva chi
          t = get_time();
          computeActiveErrors();
          tempChi = activeChi2();
          ...
          // Calculo del factor de dampering
          ...
          } else
          {
            ...
            ni *= 2;
            t = get_time();
            // restore the last state before trying to optimize
            pop(_activeVertices);
          }
          qmax++;
        } while (rho < 0 && qmax < _maxTrialsAfterFailure && ! terminate());

        if (qmax == _maxTrialsAfterFailure || fabs(rho) < 1e-15 /*rho == 0*/)
           i = iterations;
