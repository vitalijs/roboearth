...
{
  boost::shared_ptr<const sensor_msgs::Image> ros_img_ptr (new sensor_msgs::Image(req.Image));

//  IplImage *im;
  cv_bridge::CvImageConstPtr cvcp;
  if(m_camera.usesBayer())
  {
    // m_camera converts the image
//    im = cv_bridge::toCvShare(ros_img_ptr, "passthrough");
	  cvcp = cv_bridge::toCvShare(ros_img_ptr,"passthrough");
  }
  else
  {
    // ros converts the image
//    im = cv_bridge::toCvShare(ros_img_ptr, "mono8");
	  cvcp = cv_bridge::toCvShare(ros_img_ptr,"mono8");
  }

//  cv::Mat ret = cv::Mat (im).clone ();
  cv::Mat ret = cvcp.get()->image.clone();
  ...
  int max_points_per_object) const
{
  ...
  for(it = detections.begin(); it != detections.end(); ++it)
  {
    ...
    if(!i_remove.empty())
    {
      ...
      DUtils::STL::removeIndices(it->octave, i_remove, false);
    }
  ...
  re_msgs::DetectedObject& detection, int max_points) const
{
  ...
  if(max_points <= 1)
  {
    ...
    detection.octave.resize(max_points);
    return;
  }
  ...
  DUtils::STL::removeIndices(detection.octave, i_remove, false);
  /*
