...
static cv_bridge::CvImage m_bridge;
...
//	IplImage * img_ipl =  m_bridge.imgMsgToCv(msg, "passthrough");
  cv_bridge::CvImageConstPtr cvp = cv_bridge::toCvShare(msg,"passthrough");
	cv::Mat image = cvp.get()->image.clone();
...
//	IplImage img(image);
	...
	try{
		m_bridge.image=image;
		m_bridge.encoding = "passthrough";
//		sensor_msgs::Image::Ptr ros_img_ptr = m_bridge.cvToImgMsg(&img, "passthrough");
		sensor_msgs::Image::Ptr ros_img_ptr = m_bridge.toImageMsg();
